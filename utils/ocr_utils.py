...
import cv2, base64, datetime, requests, json, numpy as np
...
# call CTPN得到坐标
def call_ocr(image, ctpn_url=CFG['local']['url'] + "ocr"):
...
#     ]
# }
def call_crnn(images, crnn_url=CFG['local']['url'] + "crnn"):
    post_data = []
    for _img in images:
...
# 图像缩放，高度都是32
def resize_batch_image(image_list: list):
    out_height, out_width = conf.CRNN_INPUT_SIZE
    out_width = out_width * 8  # 为了配合resnet正方形卷积
    target_image_list = []
    seq_length_list = []
    # 强制缩放
    for img in image_list:
        out_img, image_width_no_padding = resize_by_height_with_padding(img, out_height, out_width)
        target_image_list.append(out_img)
        seq_length_list.append(math.ceil(image_width_no_padding / 4))
    return target_image_list, seq_length_list


def resize_by_height_with_padding(image, target_height, target_width):
    h, w, _ = image.shape
    # logger.info("小图大小：%r",image.shape)
    scale = target_height / h
    target_image = cv2.resize(image, None, fx=8, fy=1, interpolation=cv2.INTER_AREA)  # 先把宽缩放为8倍
    target_image = cv2.resize(target_image, None, fx=scale, fy=scale, interpolation=cv2.INTER_AREA)
    ...
    if w > target_width:
        ...
        return target_image, target_width/8
