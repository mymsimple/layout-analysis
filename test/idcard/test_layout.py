...
def analysis(result):
    ...
    for r in res:
        y = r['pos']['y']
        if abs(y - current_max_y) > GAP/4 and y - current_max_y < 4*GAP:
            row += 1
            current_max_y = y
            r['row'] = row
        else:
...
def cut_contours(image):
    return test_detect(image)

def cut_and_recognize(image_path):
    positions,gray = test_detect(image_path) # 切图
    ...
    for i in range(len(positions)):
        ...
        cv2.imwrite('.idcard//debug/' + str(i) + '.jpg', img)
...
def main(image_path):
    result = cut_and_recognize(image_path)
    analysis(result)


if __name__ == "__main__":
    image_path = './data/cut/2BCAFB4E-2FD8-CD58-35F2-3B8C4DEB5665B1-1.jpg'
